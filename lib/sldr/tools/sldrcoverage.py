...
def main():
    ...
    for r in sorted(results, key=lambda x:(-sum(100. * x[1].get(k, 0)/reseng.get(k,0) for k in levelnames), x[0])):
        if len(r[1]):
            outcsv.writerow([r[0], r[2]] + ["{:.2f}".format(100. * r[1].get(k, 0) / reseng.get(k,0)) for k in levelnames])
